...
export default function ProductCard({ product, index = 0 }) {
  ...
  const handleMouseMove = (e) => {
    ...
    setTilt({
      x: (ny - 0.5) * -16,   // pitch  (tilt toward cursor)
      y: (nx - 0.5) *  16,   // yaw
      mx: nx * 100,
      my: ny * 100,
    })
  ...
  const handleMouseLeave = () => setTilt({ x: 0, y: 0, mx: 50, my: 50 })
    ...
    >
      ...
      >
        <Link to={`/products/${product.id || product._id}`} className="group block">
          ...
          {/* Image container */}
          <div
            className="relative overflow-hidden bg-brand-surface aspect-[4/5] border border-brand-border/50 group-hover:border-brand-gold/40 transition-all duration-500"
            style={{ clipPath: 'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 0 100%)' }}
          >
            {/* Moving gloss sheen */}
            <div
              className="card-tilt-sheen"
              style={{ '--mx': `${tilt.mx}%`, '--my': `${tilt.my}%` }}
            />
            ...
            {/* Corner accents — translateZ for depth illusion */}
            <div
              className="absolute top-0 right-0 w-5 h-5 border-t border-r border-brand-gold/50 z-10 pointer-events-none transition-all duration-300 group-hover:border-brand-gold group-hover:w-7 group-hover:h-7"
              style={{ transform: 'translateZ(8px)' }}
            />
            <div
              className="absolute bottom-0 left-0 w-5 h-5 border-b border-l border-brand-gold/25 z-10 pointer-events-none transition-all duration-300 group-hover:border-brand-gold/60 group-hover:w-7 group-hover:h-7"
              style={{ transform: 'translateZ(8px)' }}
            />
            ...
            <div className="absolute inset-0 shadow-[inset_0_0_30px_rgba(184,120,80,0)] group-hover:shadow-[inset_0_0_30px_rgba(184,120,80,0.1)] transition-shadow duration-500 pointer-events-none" />

            {product.isNew && (
              <span
                className="absolute top-3 left-3 bg-brand-cream text-brand-black text-[9px] px-2.5 py-1 font-medium tracking-[0.2em] uppercase z-10"
            ...
            {product.originalPrice && (
              <span
                className="absolute top-3 left-3 bg-brand-gold text-white text-[9px] px-2.5 py-1 font-medium tracking-[0.2em] uppercase z-10"
                style={{ transform: 'translateZ(12px)' }}
              >
                Sale
              </span>
            ...
            <div
              ...
              style={{ transform: tilt.x !== 0 ? 'translateY(0%) translateZ(10px)' : undefined }}
            >
              <button
                onClick={handleAddToCart}
                className="w-full bg-brand-cream text-brand-black py-3 text-xs font-medium tracking-[0.2em] uppercase hover:bg-brand-gold hover:text-white transition-colors duration-200"
              >
                Add to Cart
              </button>
            </div>
